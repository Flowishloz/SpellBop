...
func _run() -> void:
	...
	# A docked card's own center must resolve to a real slot. (Index 0 = the ATTACK card, in hand here
	# because nothing is staged yet so it is off cooldown; DEFENSE/COUNTER stay hidden until their cue.)
